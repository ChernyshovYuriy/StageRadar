...
CACHE_FILE = Path(__file__).parent / "market_cache.json"  # fixed: always py/market_cache.json
...
def _save_cache(data: dict):
    # _report_to_dict already converts all numpy/pandas scalars via _to_builtin;
    # _json_default is a safety net for anything that slipped through.
    CACHE_FILE.write_text(
        json.dumps({**data, "_cached_at": time.time()}, indent=2, default=_json_default)
    )
...
# ──────────────────────────────────────────────────────────────────────────────
# App — lifespan defined first so it can be passed to FastAPI()
# ──────────────────────────────────────────────────────────────────────────────
...
app = FastAPI(title="Market Stage Dashboard API", lifespan=lifespan)
...
def trigger_analysis():
    ...
    threading.Thread(target=_run_analysis, args=(tickers,), daemon=True).start()
